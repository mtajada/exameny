...
export const READING_LAYOUTS: ReadonlySet<RuoELayoutKey> = new Set(['ruoe-reading-mcq', 'ruoe-gapped-text', 'ruoe-multiple-matching', 'ruoe-cross-text'])

export function isReadingLayout(layout: RuoELayoutKey): boolean {
  return READING_LAYOUTS.has(layout)
}

export function isUseOfEnglishLayout(layout: RuoELayoutKey): boolean {
  return !READING_LAYOUTS.has(layout)
}
